...
contract UpkeepFlawed is Upkeep {
    ...
    function performFlawedUpkeep() public {
        ...
        try this._step1() {} catch (bytes memory error) {
            emit UpkeepError("Step 1", error);
        }

        try this._step2(msg.sender) {} catch (bytes memory error) {
            emit UpkeepError("Step 2", error);
        }

        try this._step3() {} catch (bytes memory error) {
            emit UpkeepError("Step 3", error);
        }

        try this._step4() {} catch (bytes memory error) {
            emit UpkeepError("Step 4", error);
        }

        try this._step5() {} catch (bytes memory error) {
            emit UpkeepError("Step 5", error);
        }

        try this._step6() {} catch (bytes memory error) {
            emit UpkeepError("Step 6", error);
        }

        try this._step7() {} catch (bytes memory error) {
            emit UpkeepError("Step 7", error);
        }

        try this._step8() {} catch (bytes memory error) {
            emit UpkeepError("Step 8", error);
        }

        try this._step9() {} catch (bytes memory error) {
            emit UpkeepError("Step 9", error);
        }

        try this._step10() {} catch (bytes memory error) {
            emit UpkeepError("Step 10", error);
        }

        try this._step11() {} catch (bytes memory error) {
