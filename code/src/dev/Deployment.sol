...
contract Deployment is Test {
    ...
    address public constant DEPLOYER = 0x73107dA86708c2DAd0D91388fB057EeE3E2581aF;
    ...
    //@note deploy DAO to use it for tests on forked Sepolia
    IDAO public dao = IDAO(address(0x112223db8E32d056170da32FD3Dba90adCa50159));

    //@ote the following does not work, because "exchangeConfig" is not defined on IDAO =>
    //dao is not a contract instance, but only an IDAO interface =>
    //we need to perform an assembly low level call
    //IExchangeConfig public exchangeConfig = IExchangeConfig(dao.exchangeConfig());
    ...
    //@note trick, to not include this file in the coverage analysis
    function test() public {}

    //@note check helper method to return a contract instance using a low level call on the provided contr
    function getContract(address _contract, string memory _functionName) public returns (address result) {
        ...
        //@note either use a low level solidity call or use assembly:
        // (bool success, bytes memory output) = _contract.call{gas: remainingGas}(data);
        ...
        assembly {
            success := call(
                ...
                0, // no ether
                add(data, 32), //argoffset:  input buffer (starts after the first 32 bytes in the `data` array)
                mload(data), //argsize:  input length (loaded from the first 32 bytes in the `data` array)
                add(output, 32), //return offset output buffer
                32 //return size output length is 32 bytes because address is 20 bytes
            )
        }
    ...
    function initializeContracts() public {
        ...
        //benef, start, duration
        daoVestingWallet = new VestingWallet(address(dao), uint64(block.timestamp), 60 * 60 * 24 * 365 * 10); //10 years
        ...
        //@note Transfer ownership of the newly created config files to the DAO
    ...
    function grantAccessAlice() public {
        //@note encode a sig to get bytes
        bytes memory sig = abi.encodePacked(aliceAccessSignature);

        vm.prank(address(0x1111)); //sg for alice was created with this address: 0x1111
    ...
    function finalizeBootstrap() public {
        ...
        //@note Increase current blocktime to be greater than completionTimestamp
