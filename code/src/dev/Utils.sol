...
contract Utils {
    ...
    function circulatingSALT(
        IERC20 salt,
        address emissions,
        address daoVestingWallet,
        address teamVestingWallet,
        address stakingRewardsEmitter,
        address liquidityRewardsEmitter
    ) public view returns (uint256) {
        // Don't include balances that still haven't been distributed
        return
            salt.totalSupply() -
            salt.balanceOf(emissions) -
            salt.balanceOf(daoVestingWallet) -
            salt.balanceOf(teamVestingWallet) -
            salt.balanceOf(stakingRewardsEmitter) -
            salt.balanceOf(liquidityRewardsEmitter);
    }

    // Shortcut for returning the current percentStakedTimes1000 and stakingAPRTimes1000
    function stakingPercentAndAPR(
        ISalt salt,
        IStaking staking,
        ...
        address daoVestingWallet,
        address teamVestingWallet
    ) public view returns (uint256 percentStakedTimes1000, uint256 stakingAPRTimes1000) {
        // Make sure that the InitDistribution has already happened
        if (salt.balanceOf(stakingRewardsEmitter) == 0) return (0, 0);

        uint256 totalCirculating = circulatingSALT(
            salt,
            emissions,
            daoVestingWallet,
            teamVestingWallet,
    ...
    ) public view returns (uint256 minUnstakePercent, uint256 minUnstakeWeeks, uint256 maxUnstakeWeeks) {
    ...
    ) public view returns (uint256 btcPrice, uint256 ethPrice, uint256 saltPrice, uint256 usdsPrice) {
    ...
    function nonUserPoolInfo(
        ICollateralAndLiquidity collateralAndLiquidity,
        IRewardsEmitter liquidityRewardsEmitter,
        IPools pools,
        IPoolsConfig poolsConfig,
        IRewardsConfig rewardsConfig,
    ...
    function userPoolInfo(
        ...
        view
        returns (
            uint256[] memory userCooldowns,
            uint256[] memory userPoolShares,
            uint256[] memory userRewardsForPools,
            uint256[] memory userTokenBalances
        )
    {
    ...
    function userStakingInfo(
        address wallet,
        ISalt salt,
        IStaking staking
    ) public view returns (uint256 allowance, uint256 saltBalance, uint256 xsaltBalance, uint256 pendingRewards) {
    ...
    function determineZapSwapAmount(
        uint256 reserveA,
        uint256 reserveB,
        uint256 zapAmountA,
        uint256 zapAmountB
    ) external pure returns (uint256 swapAmountA, uint256 swapAmountB) {
    ...
    function quoteAmountOut(IPools pools, IERC20[] memory tokens, uint256 amountIn) external view returns (uint256 amountOut) {
        ...
        for (uint256 i = 1; i < tokens.length; i++) {
            ...
            if (reserve0 <= PoolUtils.DUST || reserve1 <= PoolUtils.DUST || amountIn <= PoolUtils.DUST) return 0;
    ...
    function quoteAmountIn(IPools pools, IERC20[] memory tokens, uint256 amountOut) external view returns (uint256 amountIn) {
        ...
        for (uint256 i = 2; i <= tokens.length; i++) {
            ...
            if (reserve0 <= PoolUtils.DUST || reserve1 <= PoolUtils.DUST || amountOut >= reserve1 || amountOut < PoolUtils.DUST) return 0;
    ...
    function estimateAddedLiquidity(
        uint256 reservesA,
        uint256 reservesB,
        uint256 maxAmountA,
        uint256 maxAmountB,
        uint256 totalLiquidity
    ) external pure returns (uint256 addedLiquidity) {
        ...
        if ((reservesA < PoolUtils.DUST) || (reservesB < PoolUtils.DUST)) return maxAmountA + maxAmountB;
        ...
        if (addedAmountA > addedAmountB) addedLiquidity = (totalLiquidity * addedAmountA) / reservesA;
        else addedLiquidity = (totalLiquidity * addedAmountB) / reservesB;
    }

    function stableNonUserData(
        IStableConfig stableConfig,
        IStakingConfig stakingConfig,
        IERC20 usds
    )
        external
        view
        returns (
            uint256 minimumCollateralRatioPercent,
            uint256 modificationCooldown,
            uint256 big_minimumCollateralValueForBorrowing,
            uint256 big_usdsSupply
        )
    {
    ...
    function stableUserData(
        ICollateralAndLiquidity collateralAndLiquidity,
        address wallet
    ) external view returns (uint256 big_maxBorrowableUSDS, uint256 big_borrowedUSDS, uint256 big_maxWithdrawableCollateral) {
