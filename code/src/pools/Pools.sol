...
contract Pools is IPools, ReentrancyGuard, PoolStats, ArbitrageSearch, Ownable {
    ...
    constructor(IExchangeConfig _exchangeConfig, IPoolsConfig _poolsConfig) ArbitrageSearch(_exchangeConfig) PoolStats(_exchangeConfig, _poolsConfig) {}

    // This will be called only once - at deployment time
    //@note certain refs may not be available during deployment of the contract (because those contracts may be deployed later) =>
    //so, we cant set those refs in the constructor =>
    //add an onlyOwner (deployer) function & renounceOwnership after calling it, so, it can't be called again.
    ...
    function startExchangeApproved() external nonReentrant {
        require(
            //@note we could add a ref to bootstrapBallot to the constr, but thhen we could end up with tons of SVs =>
            //add params that are used frequently to a global config contract & access all params from that config =>
            //that way we only need 1 SV: exchangeConfig
            msg.sender == address(exchangeConfig.initialDistribution().bootstrapBallot()),
            "Pools.startExchangeApproved can only be called from the BootstrapBallot contract"
        );
    ...
    function _addLiquidity(
    ...
    ) internal returns (uint256 addedAmount0, uint256 addedAmount1, uint256 addedLiquidity) {
        ...
        if ((reserve0 == 0) || (reserve1 == 0)) {
            // Update the reserves
            //@note if user provides wrong token rations, there will be an arb possibility & the user risks to loose some funds =>
            //user has to do correct token calc on his own
        ...
        //@note amount added to pool must be prop to current reserves T1/T2 = R1/R2 =>
        //proportionalB would be prop to maxAmount0, but must be <= maxAmount1
        ...
        //@note both calc should provide more or less the same value => using the larger amount is more fav for protocol & less fav for user =>
        //because solidity rounds to 0; totalLiquidity == #T0 + #T1 , reserve0 == amount of T0
        //tot: 100+100 ; added: 10+10 => addedLiquidity = 200*10/110 = 18.18 => new totalLiquidity = 220 => user share (20) = 9% => 9% from 220 = 19.8Token
        //rounded down to 19 - user provided 20 => small loss
        if (addedAmount0 > addedAmount1) addedLiquidity = (totalLiquidity * addedAmount0) / reserve0;
    ...
    //@note totalLiquidity is req to calc user shares => cannot be provided by user => func called by collateralAndLiquidity
    function addLiquidity(
        ...
        //@note don't allow to add very small amounts
        ...
        if (flipped) (addedAmountB, addedAmountA, addedLiquidity) = _addLiquidity(poolID, maxAmountB, maxAmountA, totalLiquidity);
        else (addedAmountA, addedAmountB, addedLiquidity) = _addLiquidity(poolID, maxAmountA, maxAmountB, totalLiquidity);
    ...
    function removeLiquidity(
        ...
        if (flipped) (reclaimedA, reclaimedB) = (reclaimedB, reclaimedA);
    ...
    function _determineSwapAmountInValueInETH(IERC20 swapTokenIn, uint256 swapAmountIn) internal view returns (uint256 swapAmountInValueInETH) {
        if (address(swapTokenIn) == address(weth)) return swapAmountIn;
        ...
        if ((reservesWETH < PoolUtils.DUST) || (reservesTokenIn < PoolUtils.DUST)) return 0; // can't arbitrage as there are not enough reserves to determine swapAmountInValueInETH
    ...
    function _attemptArbitrage(IERC20 swapTokenIn, IERC20 swapTokenOut, uint256 swapAmountIn) public returns (uint256 arbitrageProfit) {
    ...
    function swap(
        ...
        uint256 minAmountOut,
        uint256 deadline
    ) external nonReentrant ensureNotExpired(deadline) returns (uint256 swapAmountOut) {
    ...
    function depositDoubleSwapWithdraw(
        IERC20 swapTokenIn,
        IERC20 swapTokenMiddle,
        IERC20 swapTokenOut,
        uint256 swapAmountIn,
        uint256 minAmountOut,
        uint256 deadline
    ...
    function getPoolReserves(IERC20 tokenA, IERC20 tokenB) public view returns (uint256 reserveA, uint256 reserveB) {
        ...
        if (flipped) (reserveA, reserveB) = (reserveB, reserveA);
    }
