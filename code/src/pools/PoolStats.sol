...
abstract contract PoolStats is IPoolStats {
    ...
    function clearProfitsForPools() external {
        ...
        for (uint256 i = 0; i < poolIDs.length; i++) _arbitrageProfits[poolIDs[i]] = 0;
    }
    ...
    function _poolIndex(IERC20 tokenA, IERC20 tokenB, bytes32[] memory poolIDs) internal pure returns (uint64 index) {
        ...
        for (uint256 i = 0; i < poolIDs.length; i++) {
            if (poolID == poolIDs[i]) return uint64(i);
        }
    ...
    //@note view without returns statement => arr is provided in the arg list by caller => function modified that arr =>
    //caller has access too updated arr - although it was specified as func arg!
