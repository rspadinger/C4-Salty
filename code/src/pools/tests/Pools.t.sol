...
contract TestPools2 is Deployment {
    ...
    constructor() {
        ...
        if (keccak256(bytes(vm.envString("COVERAGE"))) == keccak256(bytes("ye"))) initializeContracts();
    ...
    function testRemoveLiquidity() public {
        ...
        for (uint256 i = 0; i < 20; i++) _testRemoveLiquidity();
    }
    ...
    function _checkNumbersClose(uint256 x, uint256 y) public pure returns (bool) {
        if (x > y) return ((x * 999999) / 1000000) < y;

        if (x < y) return ((y * 999999) / 1000000) < x;
    ...
    function _checkZapping(uint8 decimals0, uint8 decimals1, uint256 initialLiquidity0, uint256 initialLiquidity1, uint256 zapAmount0, uint256 zapAmount1) internal {
        ...
        collateralAndLiquidity.depositLiquidityAndIncreaseShare(
            token0,
            token1,
            initialLiquidity0 * 10 ** decimals0,
            initialLiquidity1 * 10 ** decimals1,
            0,
            block.timestamp,
            false
        );
        ...
        {
            (, , uint256 addedLiquidity) = collateralAndLiquidity.depositLiquidityAndIncreaseShare(
                token0,
                token1,
                zapAmount0 * 10 ** decimals0,
                zapAmount1 * 10 ** decimals1,
                0,
                block.timestamp,
                true
            );
        ...
        if (reclaimedA > zapAmount0 * 10 ** decimals0) pools.depositSwapWithdraw(token0, token1, reclaimedA - zapAmount0 * 10 ** decimals0, 0, block.timestamp);

        if (reclaimedB > zapAmount1 * 10 ** decimals1) pools.depositSwapWithdraw(token1, token0, reclaimedB - zapAmount1 * 10 ** decimals1, 0, block.timestamp);
    ...
    function _checkZappingDust(uint8 decimals0, uint8 decimals1, uint256 initialLiquidity0, uint256 initialLiquidity1, uint256 zapAmount0, uint256 zapAmount1) internal {
        ...
        // Swap back and see if things are where we started
        if (reclaimedA > zapAmount0) pools.depositSwapWithdraw(token0, token1, reclaimedA - zapAmount0, 0, block.timestamp);

        if (reclaimedB > zapAmount1) pools.depositSwapWithdraw(token1, token0, reclaimedB - zapAmount1, 0, block.timestamp);
