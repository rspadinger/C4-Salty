...
contract Upkeep is IUpkeep, ReentrancyGuard {
    ...
    constructor(
        ...
        ICollateralAndLiquidity _collateralAndLiquidity,
    ...
    function step2(address receiver) public onlySameContract {
        ...
        uint256 rewardAmount = (withdrawnAmount *
            daoConfig.upkeepRewardPercent()) / 100;
    ...
    function step3() public onlySameContract {
        ...
        uint256 amountOfWETH = (wethBalance *
            stableConfig.percentArbitrageProfitsForStablePOL()) / 100;
    ...
    function step4() public onlySameContract {
        ...
        uint256 amountOfWETH = (wethBalance *
            daoConfig.arbitrageProfitsPercentPOL()) / 100;
    ...
    function step8() public onlySameContract {
        uint256 timeSinceLastUpkeep = block.timestamp -
            lastUpkeepTimeRewardsEmitters;

        saltRewards.stakingRewardsEmitter().performUpkeep(timeSinceLastUpkeep);
        saltRewards.liquidityRewardsEmitter().performUpkeep(
            timeSinceLastUpkeep
        );
    ...
    function step10() public onlySameContract {
        VestingWallet(payable(exchangeConfig.daoVestingWallet())).release(
            address(salt)
        );
    }
    ...
    function step11() public onlySameContract {
        uint256 releaseableAmount = VestingWallet(
            payable(exchangeConfig.teamVestingWallet())
        ).releasable(address(salt));
        ...
        VestingWallet(payable(exchangeConfig.teamVestingWallet())).release(
            address(salt)
        );

        salt.safeTransfer(
            exchangeConfig.managedTeamWallet().mainWallet(),
            releaseableAmount
        );
    }
    ...
    function performUpkeep() public nonReentrant {
        ...
        try this.step1() {} catch (bytes memory error) {
            emit UpkeepError("Step 1", error);
        }

        try this.step2(msg.sender) {} catch (bytes memory error) {
            emit UpkeepError("Step 2", error);
        }

        try this.step3() {} catch (bytes memory error) {
            emit UpkeepError("Step 3", error);
        }

        try this.step4() {} catch (bytes memory error) {
            emit UpkeepError("Step 4", error);
        }

        try this.step5() {} catch (bytes memory error) {
            emit UpkeepError("Step 5", error);
        }

        try this.step6() {} catch (bytes memory error) {
            emit UpkeepError("Step 6", error);
        }

        try this.step7() {} catch (bytes memory error) {
            emit UpkeepError("Step 7", error);
        }

        try this.step8() {} catch (bytes memory error) {
            emit UpkeepError("Step 8", error);
        }

        try this.step9() {} catch (bytes memory error) {
            emit UpkeepError("Step 9", error);
        }

        try this.step10() {} catch (bytes memory error) {
            emit UpkeepError("Step 10", error);
        }

        try this.step11() {} catch (bytes memory error) {
            emit UpkeepError("Step 11", error);
        }
    }
    ...
    function currentRewardsForCallingPerformUpkeep()
        public
        view
        returns (uint256)
    {
