...
contract TestSaltRewards is SaltRewards {
    constructor(
        IRewardsEmitter _stakingRewardsEmitter,
        IRewardsEmitter _liquidityRewardsEmitter,
    ...
    function sendLiquidityRewards(
        uint256 liquidityRewardsAmount,
        uint256 directSaltUSDSRewardsAmount,
        bytes32[] memory poolIDs,
        uint256[] memory profitsForPools
    ) public {
        uint256 totalProfits = 0;
        for (uint256 i = 0; i < poolIDs.length; i++)
            totalProfits += profitsForPools[i];

        _sendLiquidityRewards(
            liquidityRewardsAmount,
            directSaltUSDSRewardsAmount,
            poolIDs,
            profitsForPools,
            totalProfits
        );
    ...
    function sendInitialLiquidityRewards(
        uint256 liquidityBootstrapAmount,
        bytes32[] memory poolIDs
    ) public {
