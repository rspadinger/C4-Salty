...
contract TestUniswapFeed is CoreUniswapFeed {
    ...
    constructor(
        ...
        IERC20 _weth,
        IERC20 _usdc,
        address _UNISWAP_V3_WBTC_WETH,
        address _UNISWAP_V3_WETH_USDC
    )
        CoreUniswapFeed(
            _wbtc,
            _weth,
            _usdc,
            _UNISWAP_V3_WBTC_WETH,
            _UNISWAP_V3_WETH_USDC
        )
    {}
    ...
    function getUniswapTwapWei(
        ...
        if (pool == UNISWAP_V3_WBTC_WETH)
            if (forcedTWAP_WBTC_WETH != 0) return forcedTWAP_WBTC_WETH;

        if (pool == UNISWAP_V3_WETH_USDC)
            if (forcedTWAP_WETH_USDC != 0) return forcedTWAP_WETH_USDC;
