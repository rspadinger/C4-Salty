...
abstract contract Liquidity is ILiquidity, StakingRewards {
    ...
    bytes32 public immutable collateralPoolID;
    ...
    function _depositLiquidityAndIncreaseShare(
        ...
        if (useZapping) (maxAmountA, maxAmountB) = _dualZapInLiquidity(tokenA, tokenB, maxAmountA, maxAmountB);
        ...
        if (addedAmountA < maxAmountA) tokenA.safeTransfer(msg.sender, maxAmountA - addedAmountA);

        if (addedAmountB < maxAmountB) tokenB.safeTransfer(msg.sender, maxAmountB - addedAmountB);
    ...
    //@note user calls this function to deposit into pool => on the UI, user selects pool or token pair and amounts to deposit =>
    //minLiquidityReceived is calculated offchain by calling: Pools::getPoolReserves
    function depositLiquidityAndIncreaseShare(
        ...
        uint256 deadline,
        bool useZapping
    ) external nonReentrant ensureNotExpired(deadline) returns (uint256 addedAmountA, uint256 addedAmountB, uint256 addedLiquidity) {
    ...
    //@note user calls this function to withdraw tokens from pool => on the UI, user selects pool or token pair and liquidityToWithdraw =>
    //minReclaimedA & B is calculated offchain by calling: Pools::getPoolReserves
    function withdrawLiquidityAndClaim(
        ...
        uint256 minReclaimedB,
        uint256 deadline
    ) external nonReentrant ensureNotExpired(deadline) returns (uint256 reclaimedA, uint256 reclaimedB) {
