...
contract CollateralAndLiquidity is Liquidity, ICollateralAndLiquidity {
    ...
    IStableConfig public immutable stableConfig;
    IPriceAggregator public immutable priceAggregator;
    IUSDS public immutable usds;
    IERC20 public immutable wbtc;
    IERC20 public immutable weth;
    ILiquidizer public immutable liquidizer;

    // Cached for efficiency
    uint256 public immutable wbtcTenToTheDecimals;
    uint256 public immutable wethTenToTheDecimals;

    // Keeps track of wallets that have borrowed USDS (so that they can be checked easily for sufficient collateral ratios)
    EnumerableSet.AddressSet private _walletsWithBorrowedUSDS; //@note EnumerableSet
    ...
    constructor(
        ...
        IStableConfig _stableConfig,
        IPriceAggregator _priceAggregator,
        ILiquidizer _liquidizer
        ...
        wbtcTenToTheDecimals = 10 ** IERC20Metadata(address(wbtc)).decimals(); //@note access decimals of a token via Metadata
    ...
    function withdrawCollateralAndClaim(
        ...
        uint256 deadline
    ) external nonReentrant ensureNotExpired(deadline) returns (uint256 reclaimedWBTC, uint256 reclaimedWETH) {
        ...
        //collateralToWithdraw == number of user shares
    ...
    function repayUSDS(uint256 amountRepaid) external nonReentrant {
        ...
        if (usdsBorrowedByUsers[msg.sender] == 0) _walletsWithBorrowedUSDS.remove(msg.sender);
    ...
    //@note returns the number of user shares that can be withdrawn
    function maxWithdrawableCollateral(address wallet) public view returns (uint256) {
        ...
        if (userCollateralValue <= requiredCollateralValueAfterWithdrawal) return 0;
        ...
        // return allUserShares * maxUSDToWithdraw / collateralInUSD => fraction of shares that correspond with maxUSDToWithdraw
    ...
    function maxBorrowableUSDS(address wallet) public view returns (uint256) {
        ...
        if (userShareForPool(wallet, collateralPoolID) == 0) return 0;

        // The user's current collateral value will determine the maximum amount that can be borrowed
        uint256 userCollateralValue = userCollateralValueInUSD(wallet);

        if (userCollateralValue < stableConfig.minimumCollateralValueForBorrowing()) return 0;
        ...
        if (usdsBorrowedByUsers[wallet] >= maxBorrowableAmount) return 0;
    ...
    function canUserBeLiquidated(address wallet) public view returns (bool) {
        ...
        if (usdsBorrowedAmount == 0) return false;
    ...
    function findLiquidatableUsers(uint256 startIndex, uint256 endIndex) public view returns (address[] memory) {
        ...
        if (totalCollateralValue != 0)
            //@note EnumerableSet iteration only used in view
            for (uint256 i = startIndex; i <= endIndex; i++) {
                ...
                //@note totalCollateralShares represent BTC+ETH collateral
                ...
                if (userShareForPool(wallet, collateralPoolID) < minCollateral) liquidatableUsers[count++] = wallet;
            }
        ...
        for (uint256 i = 0; i < count; i++) resizedLiquidatableUsers[i] = liquidatableUsers[i];

        return resizedLiquidatableUsers;
    }

    //@note using an EnumerableSet => use view that gets number of elements in Set => use wrapper function to return array
    function findLiquidatableUsers() external view returns (address[] memory) {
        if (numberOfUsersWithBorrowedUSDS() == 0) return new address[](0); //@note return empty array
