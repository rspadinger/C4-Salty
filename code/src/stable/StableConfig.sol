...
contract StableConfig is IStableConfig, Ownable {
    ...
    event MinimumCollateralValueForBorrowingChanged(
        uint256 newMinimumCollateralValue
    );
    event InitialCollateralRatioPercentChanged(
        uint256 newInitialCollateralRatioPercent
    );
    event MinimumCollateralRatioPercentChanged(
        uint256 newMinimumCollateralRatioPercent
    );
    event PercentArbitrageProfitsForStablePOLChanged(
    ...
    function changeRewardPercentForCallingLiquidation(
        bool increase
    ) external onlyOwner {
        if (increase) {
            ...
            uint256 remainingRatioAfterReward = minimumCollateralRatioPercent -
                rewardPercentForCallingLiquidation -
                1;

            if (
                remainingRatioAfterReward >= 105 &&
        ...
        emit RewardPercentForCallingLiquidationChanged(
            rewardPercentForCallingLiquidation
        );
    }

    function changeMaxRewardValueForCallingLiquidation(
        bool increase
    ) external onlyOwner {
        if (increase) {
        ...
        emit MaxRewardValueForCallingLiquidationChanged(
            maxRewardValueForCallingLiquidation
        );
    }

    function changeMinimumCollateralValueForBorrowing(
        bool increase
    ) external onlyOwner {
        if (increase) {
        ...
        emit MinimumCollateralValueForBorrowingChanged(
            minimumCollateralValueForBorrowing
        );
    }

    function changeInitialCollateralRatioPercent(
        bool increase
    ) external onlyOwner {
        if (increase) {
        ...
        emit InitialCollateralRatioPercentChanged(
            initialCollateralRatioPercent
        );
    }

    function changeMinimumCollateralRatioPercent(
        bool increase
    ) external onlyOwner {
        if (increase) {
        ...
        emit MinimumCollateralRatioPercentChanged(
            minimumCollateralRatioPercent
        );
    }

    function changePercentArbitrageProfitsForStablePOL(
        bool increase
    ) external onlyOwner {
        if (increase) {
        ...
        emit PercentArbitrageProfitsForStablePOLChanged(
